"""
process_failed_leads.py

Reads leads from Google Sheet where has_website=TRUE but color_primary
is still empty (scraping failed or was never attempted). Assigns curated
color palettes, generates slugs, sets live_url, writes back to the sheet,
and upserts to Supabase.

Run: venv/bin/python process_failed_leads.py
"""
...
def main():
    ...
    # Eligible: has_website=TRUE, no color extracted yet, has a business name
    eligible = [
        (i + 2, row)
        for i, row in enumerate(data_rows)
        if cell(row, "has_website").upper() == "TRUE"
        and not cell(row, "color_primary")
        and cell(row, "business_name")
    ]

    total = len(eligible)
    print(f"Found {total} failed/unprocessed leads to assign palettes to.\n")
    if total == 0:
        print("Nothing to do.")
        return
    ...
    # Write back to sheet in chunks of 500
